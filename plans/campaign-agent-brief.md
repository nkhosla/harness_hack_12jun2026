...
This is the differentiator: not one briefing for one event, but a proactive, prioritized set of recommended events — the engine for community outreach and relationship-building. Sentiment isn't a readout; it's the **generative seed**. A water-quality concern in Carrboro and a school-funding fight in Caswell County aren't two facts on a dashboard — they're two different events, in two different places, with two different forecasts and two different audiences, each handled by its own Event Architect.
...
Use a region you actually have data for. **NC HD-50 (Caswell + Orange counties, Chapel Hill/Carrboro)** is the pick — official precinct-level turnout from the NC State Board of Elections (already in `data/turnout/`), multiple distinct areas (college town + rural county, so the per-event weather/precinct variation is real, not contrived), and no connection to anyone's actual campaign work. All data is aggregate public election results — no voter file, no PII.
...
4. **(1:20) The slate lands.** A ranked board of proposed events. Read two off it: "Carrboro, water quality, rain forecast → indoor listening session at [venue], target these soft precincts. Caswell County, school funding, clear skies → outdoor rally at [park], target these voters." Same system, different place, different weather, different audience.
